...
const Customers = () => {
  ...
  if (error) {
    ...
    return (
      <DashboardLayout currentPage="customers">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-lg text-red-600">{t('customers.failed-load')}</div>
        </div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout currentPage="customers">
      <div className="space-y-6">
        ...
        <div className="relative max-w-md">
          ...
          <Input 
            placeholder={t('customers.search-customers')}
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="bg-gray-100">
            <TabsTrigger value="all">{t('customers.all')} ({customers.length})</TabsTrigger>
          ...
          <TabsContent value="all" className="mt-6">
            <div className="bg-white rounded-lg shadow-sm border">
              <div className="p-6 border-b">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-semibold">{t('customers.title')} ({customers.length})</h2>
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">{t('customers.customer')}</th>
                    <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">{t('common.email').toUpperCase()}</th>
                    <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">{t('customers.joined')}</th>
                    <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">{t('common.status').toUpperCase()}</th>
                    <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">{t('common.actions').toUpperCase()}</th>
                  </tr>
                </thead>
